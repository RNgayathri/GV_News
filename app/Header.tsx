...
function Header() {
    ...
    return (
        <header>
                ...
                <div className='flex items-center justify-end space-x-2'>
                    ...
                    <button className="flex bg-slate-900 text-white px-4 lg:px-8 py-2 lg:py-4 rounded-xl dark:bg-slate-800" type="button" onClick={() => setCtyMenu(!ctyMenu)}>
                        <span className='text-center'>
                            {countries[cty].emoji} {countries[cty].name}
                        </span><ChevronDownIcon className='h-5 w-5 ml-3 mt-1' />
                        {ctyMenu && <div id="dropdown" className="flex flex-col z-10 w-44 bg-white rounded divide-y divide-gray-500 shadow dark:bg-gray-800 absolute top-20 right-10">
            ...
            <BrowserView><NavLinks Menu={Menu} setMenu={setMenu} /></BrowserView>
            <SearchBox />
        </header >
    )
}
